...
const Button = ({ id, text, color, createRectangle, zoom, mouseFollowerRef, icon, group }) => {
    const handleButtonClick = () => {
        // Calculate the position relative to the room
        const { offsetLeft, offsetTop } = mouseFollowerRef.current;
        createRectangle((offsetLeft - 75 * zoom) / zoom, (offsetTop - 25 * zoom) / zoom, 50, color, icon, group);
    };
